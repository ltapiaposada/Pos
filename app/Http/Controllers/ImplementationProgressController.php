...
class ImplementationProgressController extends Controller
{
    public function index(): View
    {
        $steps = [
            ...
            ['label' => 'Ajustar autenticacion', 'completed' => true],
            ['label' => 'Aplicar filtros por empresa', 'completed' => true],
            ['label' => 'Crear panel global', 'completed' => true],
            ['label' => 'Crear gestion de suscripciones', 'completed' => true],
            ['label' => 'Mostrar aviso 4 dias antes', 'completed' => true],
            ['label' => 'Bloquear sistema vencido', 'completed' => true],
            ['label' => 'Probar aislamiento de datos', 'completed' => true],
        ];

        $findings = [
            'Ya existe branches y users.branch_id, por lo que la app ya separa parte de la operacion por sucursal.',
            'Las tablas operativas mas sensibles ya usan branch_id: ventas, compras, inventario, caja y devoluciones.',
            'La base ya incluye companies, company_types y company_subscriptions con migracion de datos iniciales.',
            'Las tablas maestras principales quedaron ligadas a company_id y ahora usan scopes por empresa.',
            'La autenticacion sigue sobre Breeze y Spatie Permission, pero ya reconoce company_id y el rol global system_owner.',
            'El registro publico ya asigna empresa y sucursal por defecto para no dejar usuarios huerfanos.',
            'Las tablas operativas principales y sus tablas hijas ya guardan company_id explicito ademas de branch_id.',
        ];
